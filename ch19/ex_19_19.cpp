...
public:
	...
	static Action getAvgPrice() {return &Sales_data::avg_price;}
	Sales_data() = default;
...
Sales_data report(const vector<Sales_data> &vec, const Sales_data &val) {
	Sales_data::Action pmf = Sales_data::getAvgPrice();
	return *std::find_if(vec.begin(), vec.end(), [val, pmf](const Sales_data &lhs) {
		return (lhs.*pmf)() < (val.*pmf)();
	});
}
